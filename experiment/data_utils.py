...
def create_task_composition(class_nums, num_tasks, fixed_order=False):
    ...
    class_map = {c: i for i, c in enumerate(label_array)}
    task_labels = []
...
def load_task_with_labels(x, y, labels,class_map):
    tmp = []
    for i in labels:
        tmp.append((np.where(y == i)[0]))
    idx = np.concatenate(tmp, axis=None)
    y_label = y[idx]
    y_label_map = np.array([class_map[i] for i in y_label])
    return x[idx], y_label_map
...
def setup_test_loader(test_data,args):
    test_loaders = []
    ...
    for (x_test, y_test) in test_data:
        test_dataset = dataset_transform(x_test, y_test, transform=t)
        test_loader = data.DataLoader(test_dataset, batch_size=64, shuffle=True, num_workers=8)
